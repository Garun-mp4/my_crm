...
Expose only the current semantic allowlist: `crm_list_leads`, `crm_create_lead`, `crm_transition_lead`, `crm_start_research`, `crm_run_research_job`, `crm_retry_research_job`, `crm_get_research_job`, `crm_record_research`, `crm_preview_lead_import`, `crm_import_leads`, `crm_rollback_import`, `crm_create_outreach_draft`, `crm_approve_outreach_draft`, and `crm_log_activity`, with explicit schemas and scopes. Twenty's app runtime registers logic functions internally with an `app_` prefix; the CRM MCP boundary strips that implementation detail and exposes only the public `crm_*` names. When the CRM app is installed, the MCP service uses this closed semantic set and omits Twenty's generic catalog/execute/learn bridge. A workspace without the CRM app keeps Twenty's standard compatibility tools. Do not expose arbitrary SQL, arbitrary GraphQL documents, unrestricted object CRUD, or a tool that silently sends external messages. Tool results are filtered by the caller's workspace and field permissions and include audit/correlation metadata where appropriate.
